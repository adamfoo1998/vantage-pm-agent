...
This skill is self-contained. It depends only on built-in Codex tools (the `Agent` tool with the built-in `general-purpose` agent, `Write`, and web search/fetch used inside those agents) plus `report-template.html` in this same folder. No external scripts, APIs, paid services, or other skills are required. Drop the folder into any `.Codex/skills/` directory and it works.
